...
class VRNNCell(snt.AbstractModule):
    ...
    def zero_state(self, batch_size, dtype):
        ...
        """
        return (
            self.rnn_cell.zero_state(batch_size, dtype),
    ...
    def _build(self, observations, state, mask, return_value=None):
        ...
        rnn_out, new_rnn_state = self.rnn_cell(
            rnn_inputs, rnn_state
        )  # STEP 3: o_t, h_t = RNN(phi_x(x_t-1), phi_z(z_t-1), h_t-1)
        # Create the prior and approximate posterior distributions.
        latent_dist_prior = self.prior(
            rnn_out
        )  # STEP 1: p(z_t|h_t) = phi_prior(o_t) = p(z_t|x_<t, z_<t)
        latent_dist_q = self.approx_posterior(
            rnn_out,
            targets_encoded,  # STEP 4: q(z_t|x_t) = phi_enc(o_t, phi_x(x_t))
            prior_mu=latent_dist_prior.loc,
        )
        # Sample the new latent state z and encode it.
        latent_state = latent_dist_q.sample(
            seed=self.random_seed
        )  # z_t ~ q(z_t|x_t) = phi_enc(o_t, phi_x(x_t))
        latent_encoded = self.latent_feat_extractor(latent_state)  # phi_z(z_t)

        latent_state_prior = latent_dist_prior.sample(
            seed=self.random_seed
        )  # z_t ~ q(z_t|x_t) = phi_enc(o_t, phi_x(x_t))
        latent_prior_encoded = self.latent_feat_extractor(
            latent_state_prior
        )  # phi_z(z_t)
        ...
        analytic_kl = tf.reduce_sum(
            tf.contrib.distributions.kl_divergence(latent_dist_q, latent_dist_prior),
            axis=-1,
        )
        # Create the generative dist. and calculate the logprob of the targets.
        generative_dist = self.generative(
            latent_encoded, rnn_out
        )  # STEP 2: p(x_t|z_t) = phi_dec(z_t, o_t) = p(x_t|z_<=t, x<t)
        log_p_x_given_z = tf.reduce_sum(generative_dist.log_prob(targets), axis=-1)

        generative_dist_prior = self.generative(
            latent_prior_encoded, rnn_out
        )  # STEP 2: p(x_t|z_t) = phi_dec(z_t, o_t) = p(x_t|z_<=t, x<t)

        latent_encoded_return = tf.cond(
            no_target, lambda: latent_prior_encoded, lambda: latent_encoded
        )
        ...
        if return_value is None:
            return (
                ...
                log_p_x_given_z,
                analytic_kl,
                (new_rnn_state, latent_encoded_return),
                rnn_out,
            )

        if return_value == "logits":
            dists_return = tf.cond(
                no_target,
