...
def create_eval_graph(inputs, targets, lengths, model, config):
    ...
    seq_mask = tf.transpose(
        tf.sequence_mask(lengths, maxlen=max_seq_len, dtype=tf.float32), perm=[1, 0]
    )
    if num_samples > 1:
    ...
    tas = [tf.TensorArray(tf.float32, max_seq_len, name="%s_ta" % n) for n in ta_names]
    ...
    def while_step(t, rnn_state, tas, accs, while_samples):
        ...
        log_q_z, log_p_z, log_p_x_given_z, kl, new_state, new_rnn_out, dists_return = (
            model(cur_inputs, rnn_state, cur_mask, return_value="probs")
        )

        new_sample0 = dists.sample_from_probs(
            dists_return,
        ...
        if config.bound == "fivo":
            ...
            ancestor_inds = tf.stop_gradient(
                resampling_dist.sample(
                    sample_shape=num_samples, seed=config.random_seed
                )
            )
            ...
            should_resample = tf.logical_and(
                should_resample, cur_mask[:batch_size] > 0.0
            )
            float_should_resample = tf.to_float(should_resample)
            ancestor_inds = tf.where(
                tf.tile(should_resample, [num_samples]), ancestor_inds, noresample_inds
            )
        ...
        if config.bound == "fivo":
            ...
            log_p_hat_update = tf.reduce_logsumexp(log_weights_acc, axis=0) - tf.log(
                tf.to_float(num_samples)
            )
            log_p_hat_acc += log_p_hat_update * float_should_resample
            log_weights_acc *= 1.0 - tf.tile(
    ...
    log_weights, track_sample, track_true, rnn_state_tf, rnn_latent_tf, rnn_out_tf = [
        x.stack() for x in tas
    ]
    ...
    if config.bound == "fivo":
        ...
        log_p_hat += tf.reduce_logsumexp(final_log_weights, axis=0) - tf.log(
            tf.to_float(num_samples)
        )
    ...
    return (
        ...
        log_weights,
        ll_per_t,
        final_log_weights / tf.to_float(lengths),
        rnn_state_tf,
        rnn_latent_tf,
        rnn_out_tf,
    )
...
def create_dataset_and_model(config, shuffle, repeat):
    ...
    generative_bias_init = -tf.log(1.0 / tf.clip_by_value(mean, 0.0001, 0.9999) - 1)
...
def wait_for_checkpoint(saver, sess, logdir):
    while True:
        if restore_checkpoint_if_exists(saver, sess, logdir):
            ...
            tf.logging.info(
                "Checkpoint not found in %s, sleeping for 60 seconds." % logdir
            )
            time.sleep(60)
...
def run_train(config):
    ...
    def create_loss():
        ...
        inputs, targets, mmsis, time_starts, time_ends, lengths, model = (
            create_dataset_and_model(config, shuffle=True, repeat=True)
        )
        # Compute lower bounds on the log likelihood.
        if config.bound == "elbo":
            ll_per_seq, _, _, _ = bounds.elbo(
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(device):
            ...
            with tf.train.MonitoredTrainingSession(
                ...
                cur_step = -1
                while True:
                    if sess.should_stop() or cur_step > config.max_steps:
                        break
                    if config.task > 0 and not start_training:
                        cur_step = sess.run(global_step)
                        tf.logging.info(
                            "task %d not active yet, sleeping at step %d"
