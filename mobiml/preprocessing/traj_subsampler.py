...
class TrajectorySubsampler():
    ...
    def subsample(self, min_dt_sec=10) -> Dataset:
        tqdm.pandas()
        df_clean = self.data.df.sort_values(TIMESTAMP, kind='mergesort') \
            .groupby(TRAJ_ID) \
            .progress_apply(
                lambda l: self.subsample_trajectory(l.copy(), min_dt_sec
            )
        ).reset_index(level=0, drop=True)
        self.data.df = df_clean
        return self.data

    def subsample_trajectory(self, traj, min_dt_sec=10) -> pd.DataFrame:
        # Based on: https://stackoverflow.com/a/56904899
        sumlm = np.frompyfunc(lambda a,b: a+b if a < min_dt_sec else b, 2, 1)

        traj_dt = traj[TIMESTAMP].diff().dt.total_seconds()
